counts <- function(edgelist, riskset, sender = F){
  ...
  if(sender){
    ...
    #Output
    m <- array(0, c(max(nact)-1, n, max(events)))
  ...
  for(i in 1:n){
    ...
    for(j in 1:events[i]){
      ...
      if(sender){
        ...
        snd <- which(edgelist[[i]][j,2] == riskset[riskset[,3]==i,1]) #which sender?
        dyad <- which(edgelist[[i]][j,2] == riskset[riskset[,3]==i,1] &
                       edgelist[[i]][j,3] == riskset[riskset[,3]==i,2]) #which dyad?
        m[which(snd == dyad),i,j] <- 1
...
indicators <- function(edgelist, riskset, sender = F){
  ...
  names(actor) <- c("sender", "cluster")
  actor <- actor[with(actor, order(cluster, sender)),]
  snd <- tapply(actor[,1], actor[,2], length) #number of senders per group
  receiver <- unlist((lapply(1:n, function(x) rep(x, snd[x]-1))))
  ...
  if(sender){
    ...
    #output
    ind <- matrix(0, ncol = n, nrow = nrow(actor)-n)
  ...
  if(!sender){
    ...
    m <- matrix(0, ncol = n, nrow = max(table(actor$cluster))-1)
