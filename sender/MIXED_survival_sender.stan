data {
  ...
  int J;
  //number of fixed effects for the actor intensity
  int covFA;
  ...
  int riskset[N];
  //actors data array of fixed effexts
  matrix[sum(actor),covFA] SFactor[max(events)];
...
model{
  ...
  for(i in 1:N){
    ...
    for(j in 1:events[i]){
      ...
      lambdaSnd = exp(SFactor[j][ind1,] * phi + SRactor[j][ind1,] * gamma[,i]);
