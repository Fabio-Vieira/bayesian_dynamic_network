data {
  ...
  int riskset[N];
  //data array of fixed effects
  matrix[sum(actorR),covF] SF[max(events)];
  //data array of random effects
  matrix[sum(actorR),covR] SR[max(events)];
  //array containing the multinomial counts of events
  int counts[max(actorR),N,max(events)];
  //auxiliary matrix indicating which intensities to sum for the actors
  int recIntensity[max(actorR),N];
  //vector of 1's
  row_vector[N] vecAux;
}
...
model{
  ...
  for(i in 1:N){
    ...
    vector[actorR[i]] lambdaDyad;
    
    ind1 = recIntensity[1:actorR[i],i];
    ...
    for(j in 1:events[i]){
      ...
      lambdaDyad = exp(SF[j][ind1,] * psi + SR[j][ind1,] * beta[,i]);
      
      //fitting the multinomial counts of events
      counts[1:actorR[i],i,j] ~ multinomial(lambdaDyad/sum(lambdaDyad));
